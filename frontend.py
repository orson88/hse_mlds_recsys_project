...
def postdude(user:str, song:str):
    ...
    post_string = f"http://localhost:8000/register?input={user}%20---%20{song}"
...
def get_user_ids():
    return requests.get(
        url="http://localhost:8000/all_users?input=gigalul"
        ).text\
            .strip('[""]')\
            .split('","')

def get_recs(option):
    result = requests.get(url=f"http://localhost:8000/predictions?user_id={option}&n=5")\
        .text.split('","')
    return result
